...
/**
 * =================================================================
 * MI ARCHIVO DE FUNCIONES CENTRALIZADAS
 * Aquí he metido todas las funciones de utilidad y de lógica de negocio
 * que uso en varias partes de la aplicación, para no repetir código.
 * =================================================================
 */

// Establezco la zona horaria para toda la aplicación a la de España.
// Así me aseguro de que todas las funciones de fecha y hora (date(), strtotime(), etc.) funcionen como espero.
 ...
 * Llamo a esta función antes de mostrar cualquier formulario para crear el token.
 */
 ...
 * Llamo a esta función al principio de cualquier script que procesa datos de un formulario (POST).
 */
function validar_csrf_token() {
    ...
    // Una vez usado, elimino el token para que se genere uno nuevo en la siguiente solicitud.
    unset($_SESSION['csrf_token']);
}

/**
 * Inicia una sesión segura con cookies configuradas adecuadamente.
 * He creado esta función para iniciar una sesión segura, configurando bien las cookies.
 */
...
/**
 * Esta función me calcula la antigüedad de un usuario en días desde que se registró.
...
function antiguedad_usuario($conn, $usuario_id) {
    ...
    // Si no encuentro al usuario, retorno 0.
    return 0;
}

/**
 * Con esta función calculo el precio total de un alquiler de forma segura en el servidor, para que nadie pueda manipularlo.
 * @param mysqli $conn La conexión a la base de datos.
...
function calcular_precio_total($conn, $moto_id, $f_inicio, $f_fin) {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        // Calculo la diferencia de días entre las fechas y le sumo 1 para incluir el día de inicio.
        $dias = (strtotime($f_fin) - strtotime($f_inicio)) / (60 * 60 * 24) + 1;
        // Retorno el precio total.
        return $precio_dia * $dias;
    }
    // Si la moto no se encuentra, retorno 0.00.
    return 0.00;
}

/**
 * Esta función me calcula el total que ha gastado un usuario en alquileres que ya están 'confirmados' o 'finalizados'.
 * @param mysqli $conn La conexión a la base de datos.
 * @param int $usuario_id El ID del usuario.
 ...
 * La uso para saber si ya hay una reserva o alquiler activo en un rango de fechas para una moto.
 * @param mysqli $conn La conexión a la base de datos.
...
/**
 * Este es mi procedimiento principal para actualizar el estado de todo el sistema.
 * Lo ejecuto al cargar páginas clave como el perfil o el panel de admin.
 * @param mysqli $conn La conexión a la base de datos.
 */
function actualizar_sistema_completo($conn) {
    // 1. Paso los alquileres 'confirmados' a 'en_curso' si la fecha actual está dentro de su rango.
    $conn->query("UPDATE alquileres SET estado = 'en_curso' WHERE fecha_inicio <= CURDATE() AND fecha_fin >= CURDATE() AND estado = 'confirmado'");
    
    // 2. Paso los alquileres 'confirmados' o 'en_curso' a 'finalizado' si su fecha de fin ya ha pasado.
    $conn->query("UPDATE alquileres SET estado = 'finalizado' WHERE fecha_fin < CURDATE() AND estado IN ('confirmado', 'en_curso')");
    
    // 3. Cancelo automáticamente los alquileres 'pendientes' si su fecha de inicio ya pasó (porque el cliente no pagó a tiempo).
    ...
    // 5. Y ahora marco como 'no disponible' solo aquellas motos que están en un alquiler activo ('confirmado' o 'en_curso').
...
/**
 * He creado esta función para simular un TRIGGER BEFORE INSERT para la tabla de usuarios.
 * Hago algunas conversiones de datos (email a minúsculas, nombre a mayúsculas) antes de la inserción.
 * @param mysqli $conn La conexión a la base de datos.
 */
...
/**
 * Esta función simula un EVENTO de base de datos que se podría ejecutar periódicamente.
 * Su objetivo es cancelar las reservas que llevan más de 24 horas en estado 'pendiente'.
 * @param mysqli $conn La conexión a la base de datos.
 */
