...
// Segundo, compruebo que la nueva contraseña y su confirmación coinciden.
if ($new_password !== $confirm_new_password) {
    header('Location: perfil_usuario.php?password_change_error=new_mismatch');
    exit();
}

// Tercero, compruebo que la nueva contraseña cumple mis requisitos de seguridad
...
// Guardo la contraseña hasheada que he recuperado de la BD en una variable.
$current_password_hash_db = $usuario['password'];

// --- 5. VERIFICACIÓN DE LA CONTRASEÑA ACTUAL ---
// Uso password_verify() para comparar de forma segura la contraseña que ha escrito el usuario en el formulario
...
// Si todo lo anterior ha ido bien, creo un nuevo hash para la nueva contraseña.
$new_password_hash = password_hash($new_password, PASSWORD_DEFAULT);

// Y finalmente, actualizo la contraseña en la base de datos, guardando el nuevo hash.
...
if (mysqli_stmt_execute($stmt_update)) {
    // Si tiene éxito, lo redirijo al perfil con un mensaje de éxito.
    header('Location: perfil_usuario.php?password_change=success');
} else {
    // Si falla, lo redirijo con un mensaje de error.
    header('Location: perfil_usuario.php?password_change_error=sql');
}

// Cierro todo para liberar los recursos.
