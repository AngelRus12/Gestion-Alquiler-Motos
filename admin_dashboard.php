...
/**
 * admin_dashboard.php
 * Este es mi panel de administración para gestionar usuarios, motos, alquileres y promociones.
 * - Solo yo (o un admin) puedo acceder.
 * - He programado que actualice los estados de los alquileres automáticamente y me muestre estadísticas clave.
 * - Desde aquí puedo publicar promociones con fechas de inicio/fin y activarlas o desactivarlas.
 */
...
// Incluyo mi archivo de funciones reutilizables, como la que actualiza todo el sistema.
require_once 'funciones.php';

// --- 1. CONTROL DE ACCESO ---
// Verifico si el usuario ha iniciado sesión y si tiene el rol de 'admin'.
// Si no cumple estas condiciones, lo redirijo a la página de login con un mensaje de error.
...
// Establezco el conjunto de caracteres a UTF-8 también para la conexión con la base de datos.
mysqli_set_charset($conexion, "utf8");

// --- 3. ACTUALIZACIÓN DEL SISTEMA ---
// Llamo a mi función centralizada que actualiza el estado de alquileres y motos.
// Por ejemplo, esta función pasa los alquileres 'confirmados' a 'en_curso' si su fecha de inicio ya ha llegado.
actualizar_sistema_completo($conexion);

// --- 4. GESTIÓN DE MENSAJES DE FEEDBACK ---
// Compruebo la URL en busca de parámetros 'msg' (éxito) o 'error' para mostrarme alertas.
$mensaje_html = "";
if (isset($_GET['msg'])) {
    // Uso un mensaje genérico para cubrir diferentes operaciones (crear, actualizar, eliminar).
    $mensaje_html = "<div class='alerta alerta-exito'>Operación realizada con éxito.</div>";
}
if (isset($_GET['error'])) {
    // He añadido un manejo específico para el error de auto-eliminación.
...
// c) Creo "mapas" (arrays asociativos) para tener un acceso súper rápido a los datos sin necesidad de hacer nuevas consultas.
$usuarios_map = [];
...
// d) Preparo los datos para el calendario (aquí sí obtengo todos los alquileres, sin paginación, para que el calendario esté completo).
$eventos_calendario = [];
...
foreach ($alquileres_calendario as $evento) {
    // Busco el usuario en mi mapa. Si no existe (porque fue eliminado), uso unos valores por defecto.
                                ...
                                <td>
                                    ...
                                    <?php } else { ?>
                                        <div class="info-alquilada">
                                            ...
                                            <?php
                                            // Busco un alquiler activo para esta moto en los datos que ya tengo cargados.
                                            foreach ($alquileres_todos as $alquiler_activo) {
                                                if ($alquiler_activo['moto_id'] == $moto['id'] && in_array($alquiler_activo['estado'], ['confirmado', 'en_curso'])) {
                                                    // Uso el mapa de usuarios para obtener el nombre sin hacer otra consulta.
                        ...
                        <tbody>
                            ...
                            if (!empty($alquileres_todos)) {
                                foreach ($alquileres_todos as $alquiler) {
                                    // Reutilizo los mapas que ya he creado al principio de la página.
