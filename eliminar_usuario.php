...
if (isset($_GET['id'])) {
    ...
    // 2. Impedir que un administrador se elimine a sí mismo
    if ($id_a_eliminar == $id_sesion_actual) {
        header('Location: admin_dashboard.php?error=autodelecion');
        exit();
    }

    // 3. Borrado Lógico: En lugar de DELETE, actualizamos el estado a 'inactivo'
    // Esto preserva la integridad de los datos y el historial de alquileres.
